...
use Drupal\ksp_filelibrary\FileErrorsImporter;
 ...
 *
 * @RestResource(
 *   id = "ksp_errors_library_rest_resource",
 *   label = @Translation("Ksp ErrorsLibrary"),
 ...
 * )
 */
class KspErrorsLibraryResource extends ResourceBase {
   ...
   *   The currently authenticated user.
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, array $serializer_formats, LoggerInterface $logger, FileSystemInterface $file_system, AccountInterface $current_user) {
  ...
  public function post(Request $request) {
    ...
    try {
      // Do import.
      $importer = new FileErrorsImporter($file);
      $importer->importErrorsLinks();
    }
    catch (FileException $e) {
      return new ResourceResponse(['message' => $e->getMessage()], 500);
    }
